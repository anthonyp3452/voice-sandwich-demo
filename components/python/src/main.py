...
from kitchen_manager import kitchen_manager
                ...
                if isinstance(message, ToolMessage):
                    ...
                    if tool_name == "confirm_order":
                        try:
                            ...
                            order_info = parsed.get("order", {})
                            oid = order_info.get("order_id")
                            if oid:
                                with restaurant_db.db_session() as conn:
                                    order = restaurant_db.get_order_by_id(conn, oid)
                                if order:
...
async def kitchen_websocket_endpoint(websocket: WebSocket):
    await websocket.accept()
    await kitchen_manager.register(websocket)
    try:
        with restaurant_db.db_session() as conn:
            orders = conn.execute(
                "SELECT id, session_id, status, created_at FROM orders ORDER BY created_at DESC"
            ).fetchall()
            active_orders = []
            for row in orders:
                order_data = restaurant_db.get_order_by_id(conn, row["id"])
                if order_data:
                    active_orders.append({
                        ...
                        "status": order_data["status"] if order_data["status"] != "confirmed" else "nuevo",
        ...
        while True:
            ...
            if data.get("type") == "update_status":
                ...
                with restaurant_db.db_session() as conn:
                    res = restaurant_db.update_order_status(conn, order_id, new_status)
                if res["ok"]:
                    await kitchen_manager.broadcast({
                        "type": "status_updated",
                        "orderId": order_id,
        ...
        await kitchen_manager.unregister(websocket)
