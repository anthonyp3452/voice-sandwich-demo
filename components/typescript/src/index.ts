...
const pendingItems: { item: string; quantity: number }[] = [];

const addToOrder = tool(
  async ({ item, quantity }) => {
    pendingItems.push({ item, quantity });
...
const confirmOrder = tool(
  async ({ orderSummary }) => {
    ...
    const items = pendingItems.map((p) => ({
      product_id: p.item.toLowerCase().replace(/\s+/g, "-"),
      name: p.item,
      quantity: p.quantity,
...
app.get(
  ...
  upgradeWebSocket(async () => {
    return {
      onOpen(_, ws) {
        kitchenManager.register(ws);
      },
      onMessage(event) {
        try {
          const data = JSON.parse(event.data as string);
          if (data.type === "update_status") {
            kitchenManager.updateStatus(data.orderId, data.status);
          }
        } catch {
